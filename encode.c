    ...
    while (ctx->video_queue->queue_count != 0 || ctx->video_queue->queue_finished != 1) {
        ...
        while (bytes_left > 0) {
            ...
            if (ctx->pipeline.video_decode.port_settings_changed == 1) {
                ...
                memcpy(&encoder_config.format.video, &decoder_config.format.video, sizeof(decoder_config.format.video));
                ...
                encoder_bitrate_config.nTargetBitrate = 1000000; //1 mbit 
                ...
                OERR(OMX_SetupTunnel(ctx->pipeline.video_decode.h, 131, ctx->pipeline.video_encode.h, 200));
                ...
                OMX_SendCommand(ctx->pipeline.video_decode.h, OMX_CommandPortEnable, 131, NULL);
